...
import main.java.com.tetrismultiplayer.client.engine.tetromino.TetrominoL;
...
public class TetrominoLTest
{
    ...
    {
	tetromino = new TetrominoL(new GamePanel(), Color.BLUE, 100, 100);
    }
	...
	assertEquals(60, all.get(0).getPosition().x);
	assertEquals(80, all.get(1).getPosition().x);
	assertEquals(100, all.get(2).getPosition().x);
	assertEquals(100, all.get(3).getPosition().x);

	assertEquals(100, all.get(0).getPosition().y);
	...
	assertEquals(60, all.get(0).getPosition().y);
